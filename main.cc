...
int main(int argc, char *argv[]) {
   ...
   while (fgets(buf1, 29, FP1)) {
      ...
      } else if (strcmp((const char *) buf2, (const char *) buf3) == 0 &&
                 strcmp((const char *) buf3, (const char *) buf4) == 0) {
         //std::cerr << "File CONSENSUS  FAILURE. Continuing.\n";
         sscanf((const char *) buf2, "%u %c %lx", &pr2, &rw, &address);
      } else if (strcmp((const char *) buf1, (const char *) buf3) == 0 &&
                 strcmp((const char *) buf1, (const char *) buf5) == 0) {
         //std::cerr << "File CONSENSUS  FAILURE. Continuing.\n";
         sscanf((const char *) buf3, "%u %c %lx", &pr2, &rw, &address);
      } else if (strcmp((const char *) buf1, (const char *) buf2) == 0 &&
                 strcmp((const char *) buf1, (const char *) buf5) == 0) {
         //std::cerr << "File CONSENSUS  FAILURE. Continuing.\n";
         sscanf((const char *) buf5, "%u %c %lx", &pr2, &rw, &address);
      } else if (strcmp((const char *) buf3, (const char *) buf4) == 0 &&
                 strcmp((const char *) buf4, (const char *) buf5) == 0) {
         //std::cerr << "File CONSENSUS  FAILURE. Continuing.\n";
         sscanf((const char *) buf5, "%u %c %lx", &pr2, &rw, &address);
      } else {
         std::cerr
                 << "File CONSENSUS READ FAILURE. THE INPUT FILE WAS UNABLE TO BE READ RELIABLY. PLEASE TRY RE-EXECUTING THIS APPLICATION\n";
